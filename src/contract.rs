...
pub fn set_metadata<S: Storage, A: Api, Q: Querier>(
    ...
    priority: u8,
    token_id: &str,
    _public_metadata: Option<Metadata>,
    private_metadata: Option<Metadata>,
) -> HandleResult {
    check_status(config.status, priority)?;
...
pub fn query_private_meta<S: Storage, A: Api, Q: Querier>(
    ...
    let config: Config = load(&deps.storage, CONFIG_KEY)?;
    if prep_info.viewer_raw.clone().unwrap() != config.admin {
        check_perm_core(
            deps,
            &prep_info.block,
            &prep_info.token,
...
pub fn query_possible_mints<S: Storage, A: Api, Q: Querier>(
    ...
    let remaining_tokens: u8 = match MAX_TOKENS - config.token_cnt - whitelist.len() as u32 {
        0 => 0,
        1 => 1,
        2 => 2,
        _ => 3
    ...
    // Calculate how many can be minted by user now
    if remaining_tokens == 0 && whitelisted == true {
        mint_num = 1;
    }
    else if remaining_mints >= remaining_tokens {
        mint_num = remaining_tokens;
    }
    else if remaining_mints < remaining_tokens {
